...
const TxModalInternal = ({ modalIsOpen, tx, closeModal, currencyName, vccPrice, setSendTo, history }) => {
  let txid = '';
  let type = '';
  let typeIcon = '';
  let typeColor = '';
  ...
  const openTxid = () => {
    if (currencyName === 'TAZ') {
      shell.openExternal(`https://chain.so/tx/VCCTEST/${txid}`);
    } else {
      ...
      <div className={[cstyles.verticalflex].join(' ')}>
        ...
        <div className={[cstyles.center].join(' ')}>
          ...
          <BalanceBlockHighlight
            vccValue={amount}
            usdValue={Utils.getVccToUsdString(vccPrice, Math.abs(amount))}
        ...
        <div className={[cstyles.flexspacebetween].join(' ')}>
          ...
          {type === 'send' && (
            <div>
              ...
              <div>VCC {Utils.maxPrecisionTrimmed(tx.fee)}</div>
            </div>
          )}
        ...
        {detailedTxns.map(txdetail => {
          const { bigPart, smallPart } = Utils.splitVccAmountIntoBigSmall(Math.abs(txdetail.amount));
...
const TxItemBlock = ({ transaction, currencyName, vccPrice, txClicked, addressBookMap }) => {
              ...
              <div key={address} className={cstyles.padtopsmall}>
                ...
                <div className={[styles.txamount, cstyles.right].join(' ')}>
                  ...
                  <div className={[cstyles.sublight, cstyles.small, cstyles.padtopsmall].join(' ')}>
                    {Utils.getVccToUsdString(vccPrice, Math.abs(txdetail.amount))}
                  </div>
                </div>
              </div>
            );
