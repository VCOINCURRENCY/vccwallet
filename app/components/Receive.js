...
const AddressBlock = ({ addressBalance, label, currencyName, vccPrice, privateKey, fetchAndSetSinglePrivKey }) => {
  ...
  const openAddress = () => {
    if (currencyName === 'TAZ') {
      shell.openExternal(`https://chain.so/address/VCCTEST/${address}`);
    } else {
      ...
      <AccordionItemPanel className={[styles.receiveDetail].join(' ')}>
        <div className={[cstyles.flexspacebetween].join(' ')}>
          <div className={[cstyles.verticalflex, cstyles.marginleft].join(' ')}>
            ...
            <div className={[cstyles.padtopsmall].join(' ')}>{Utils.getVccToUsdString(vccPrice, balance)}</div>
