...
export default class RPC {
  ...
  constructor(
    ...
    this.fnSetVccPrice = fnSetVccPrice;
  ...
  async configure(rpcConfig: RPCConfig) {
    ...
    if (!this.priceTimerID) {
      this.priceTimerID = setTimeout(() => this.getVccPrice(), 1000);
    }
  }
  ...
  static async getInfoObject(rpcConfig: RPCConfig) {
    ...
    info.currencyName = info.testnet ? 'TESTVCC' : 'VCC';
    info.vccPrice = null; // Setting this to null will copy over the existing price
  ...
  async createNewAddress(zaddress: boolean) {
    if (zaddress) {
      const newaddress = await RPC.doRPC('z_getnewaddress', ['sprout'], this.rpcConfig);
  ...
  setupNextVccPriceRefresh(retryCount: number, timeout: number) {
    // Every hour
    this.priceTimerID = setTimeout(() => this.getVccPrice(retryCount), timeout);
  }

  async getVccPrice(retryCount: number) {
    if (!retryCount) {
    ...
    try {
      const response = await new Promise((resolve, reject) => {
        axios('https://api.coincap.io/v2/rates/vcoin', {
          method: 'GET'
        })
          .then(r => resolve(r.data))
          .catch(err => {
      ...
      const vccData = response.data;
      if (vccData) {
        this.fnSetVccPrice(vccData.rateUsd);
        this.setupNextVccPriceRefresh(0, 1000 * 60 * 60); // Every hour
      } else {
        this.fnSetVccPrice(null);
        let timeout = 1000 * 60; // 1 minute
        if (retryCount > 5) {
          timeout = 1000 * 60 * 60; // an hour later
        }
        this.setupNextVccPriceRefresh(retryCount + 1, timeout);
      }
    } catch (err) {
      console.log(err);
      this.fnSetVccPrice(null);
